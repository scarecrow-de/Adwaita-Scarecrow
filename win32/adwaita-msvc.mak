...
_HASH=^#
NULL=
ICON_SUBDIR=share\icons\Adwaita-Scarecrow
...
!if [@for /f %s in ('dir /b /on $(SRCROOTDIR)\Adwaita-Scarecrow') do @if not "%s" == "cursors" (@echo SIZE_%s_dirs = \> %s.mak) & (@for /f %d in ('dir /b $(SRCROOTDIR)\Adwaita-Scarecrow\%s') do @echo %s/%d \>> %s.mak) & @echo ^$(NULL) >> %s.mak]
!endif
...
!if [@for /f %s in ('dir /b /on $(SRCROOTDIR)\Adwaita-Scarecrow) do @if not "%s" == "cursors" del %s.mak]
!endif
...
dir_list.py:
#	Generate a Python list of subdirs under Adwaita-Scarecrow/ for the icons
	@echo icon_dirs = [>$@
...
install: index.theme $(ADWAITA_PC_FILES)
	...
	for /f %d in ('dir /b $(SRCROOTDIR)\Adwaita-Scarecrow') do								\
	@(echo Copying files for %d...) & 										\
	@(mkdir $(PREFIX)\$(ICON_SUBDIR)\%d) & 										\
	@(if not "%d" == "cursors"											\
		(for /f %f in ('dir /b /on $(SRCROOTDIR)\Adwaita-Scarecrow\%d') do						\
		 (mkdir $(PREFIX)\$(ICON_SUBDIR)\%d\%f) &								\
		 (copy /b $(SRCROOTDIR)\Adwaita-Scarecrow\%d\%f\* $(PREFIX)\$(ICON_SUBDIR)\%d\%f))				\
	  else (copy /b $(SRCROOTDIR)\Adwaita-Scarecrow\%d\* $(PREFIX)\$(ICON_SUBDIR)\%d))
	...
	if exist %~dpx\..\lib\gdk-pixbuf-2.0\$(GDK_PIXBUF_MOD_VERSION)\loaders.cache 					\
		...
		@(for %z in (16x16 24x24 32x32 48x48 64x64 96x96) do							\
			...
			@(for /f %d in ('dir /b /on $(SRCROOTDIR)\Adwaita-Scarecrow\scalable') do					\
				@(for /f %f in ('dir /b /on $(SRCROOTDIR)\Adwaita-Scarecrow\scalable\%d') do			\
	...
	@echo Adwaita-Scarecrow icon theme install complete.
